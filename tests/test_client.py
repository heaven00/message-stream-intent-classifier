...
def create_classified_message(label: Literal["LABEL_0", "LABEL_1"], ts: datetime) -> ClassifiedMessage:
    return ClassifiedMessage(
        seqid=1,
        ts=ts,
        user="user1",
        message="test message",
        classification=CalendarClassification(label=label, score=0.9)
    )


def create_conversation(messages: list[Message], suspended=False, completed=False) -> Conversation:
    return Conversation(
        lines=messages,
...
def test_mark_completed_conversations_with_old_event_dates():
    ...
    conv_old.event_datetime = past_time
...
def test_extract_calendar_datetime_from_conversations_extracts_only_for_suspended_conversations(monkeypatch):
    ...
    monkeypatch.setattr('client.event_datetime_extractor', mock_event_datetime_extractor)
